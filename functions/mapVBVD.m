function [twix_obj,mdh] = mapVBVD(filename,varargin)
...
%  Local SLOW-MRSI package modification: Dr. Guodong Weng, University of Bern, 2026-07-04
%  
% 
...
if ~exist('filename','var') || isempty(filename)
    ...
    if ischar(filename)
        ...
        filelist = dir('*.dat');
        filesfound = 0;
        measPattern = ['(^|.*)meas_MID0*' num2str(measID) '_.*\.dat$'];
        for k=1:numel(filelist)
            if ~isempty(regexp(filelist(k).name, measPattern, 'once'))
                if filesfound == 0
...
for s=1:NScans
    ...
    %%%%%%%%%%%%%
    % % Paul 2018.03.06 Disable in case not available 
%     
%     % print reader version information
%     if s==1
%         fprintf('Reader version: %d', twix_obj{s}.image.softwareVersion); % for VD and above
% 
...
%     
    ...
    clear  tmpMdh  filePos  isCurrScan
